...
bool Game::checkWins() {
    switch(checkEnd()) {
        case 'f': return false;
        case 'd': player.incrementLosses(); return true;
            ...
            if(dealDealer()){
                switch (compareSum())
                {
                   case 'p': player.incrementWins();
                             player.addCash((player.getBet() * 2));
                             break;
                   case 'd' : player.incrementLosses();
                              break;
                    case 'n': player.addCash(player.getBet());
                              break;
                }
            }
        }
...
void Game::saveGame() {
    ...
    do{
        std::cout<<"Enter filename to save statistics: ";
        std::cin>>filename;
...
void Game::loadGame() {
    ...
    std::string path = "data/";
    do{
        std::cout<<"Enter filename to load statistics: ";
        std::cin>>filename;
        std::transform(filename.begin(), filename.end(), filename.begin(), ::tolower);
    }
    while(filename.compare("statistics") == 0);
    ...
    if(!f1.fail()) {
        ...
        while(player.getLosses() != sLosses) {
            player.incrementLosses();
        }
    }
    else {
        beginMenu(true, "File not found!");
    }
}
